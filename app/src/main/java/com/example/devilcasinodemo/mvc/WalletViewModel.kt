        ...
        viewModelScope.launch {
            ...
            runCatching {
                withContext(Dispatchers.IO) {
                    ...
                    val cards = api.getCards(userId).body()
                    Triple(wallet, cooldown, cards)
                }
            }.onSuccess { (wallet, cooldown, cards) ->

                walletAmount = wallet?.saldoDC ?: 0.0
                freeCooldownMillis = System.currentTimeMillis() + (cooldown * 1000)
                hasCards = !cards.isNullOrEmpty()

                Log.i(TAG, "Wallet fetched: saldo=${walletAmount}, cooldown=${freeCooldownMillis}, hasCards=${hasCards}")
